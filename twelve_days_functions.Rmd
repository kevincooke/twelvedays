...
author: "Kevin Cooke, Thomas Andrews, Amie Boyle, Rachael Ryan, Elijah Balderas"
date: "5/13/2021"
output:
...
```{r}
xmas <- xmas %>%
  mutate(day.num = english(Day)) %>%
  mutate(day.num = as.character(day.num)) %>%
  mutate(Full.Phrase = pmap_chr(list(num = Day,
                                num_word = day.num,
                                item = Gift.Item,
                                verb = Verb,
                                adjective = Adjective,
...
sing_line <- function(dataset, line, phrase_col){
 ...
 print(intro)
  ...
  if(line > 1){
    lyrics <- map_chr(line:2, print(glue::glue("{phrases[line:2]}")))
    print("\nand {phrases[1]}")
    # lyrics1 <- map_chr(line:2, print(glue::glue("{phrases[line:2]}")))
    #lyrics2 <- map_chr(1, print(glue::glue("\nand {phrases[1]}")))
    #lyrics <- glue::glue("{lyrics1} {lyrics2}")
  } else{
    lyrics <- map_chr(line, print(glue::glue("{phrases[1]}")))
  }
  return(lyrics)
...
#print(intro)
# if(line != 1){
# map_chr(line:2, return(str_glue("{phrases[line:2]}")))
# }
#else{
#  return(phrases[1])
#}
...
#print(intro)
#if(line == 1){
# map_chr(line:1, return(glue::glue("{phrases[1]}")))
# } 
# else{
#map_chr(line:2, return(glue::glue("{phrases[line:2]} \nand {phrases[1]}")))
#}

#print(intro)
 # map_chr(line:1, 
        # if(line == 1){
       #  (return(str_glue(phrases[1])))
        #    }
         # else {
           # (return(str_glue("{phrases[line:2]}, \nand {phrases[1]}")))
           # }
         #  )
  
  #lyrics1 <- map_chr(line:2, print(glue::glue("{phrases[line:2]}")))
  #lyrics2 <- map_chr(1, print(glue::glue("\nand {phrases[1]}")))
   #return(glue::glue("{intro} {lyrics1} {lyrics2}"))

# print(intro)
#  map_chr(line:1, 
         # if(line == 1){
        # (yay<-(str_glue(phrases[1])))
         #   }
         # else {
         #(yee<-(str_glue("{phrases[line:2]}, \nand {phrases[1]}")))
        #    }
        #   )
  #  return(str_glue("{yee} {yay}") %>%
  #  str_squish())
```
